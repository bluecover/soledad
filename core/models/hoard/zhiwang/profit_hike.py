...
class ZhiwangOrderProfitHike(EntityModel):

    table_name = 'hoard_zhiwang_profit_hike'
    cache_key = 'zhiwang:order:profit_hike:{id_}:v3'
    cache_key_by_order = 'zhiwang:order:profit_hike:order_id:{order_id}:v3'
    cache_key_by_user = 'zhiwang:order:profit_hike:user_id:{user_id}:v3'
    ...
    def display_text(self):
        ...
        return u' | '.join([s for s in [u'年化收益率 +%.2f%%' % rate if rate else '',
                                        u'抵扣 -%.2f元' % deduction if deduction else ''] if s])
    ...
    def order(self):
        ...
        return ZhiwangOrder.get(self.order_id)
    ...
    def achieve(self):
        ...
        if self.order.status is not ZhiwangOrder.Status.success:
    ...
    def renew(self):
        ...
        if self.order.status not in (
                ZhiwangOrder.Status.unpaid, ZhiwangOrder.Status.failure):
