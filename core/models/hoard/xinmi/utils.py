...
def iter_banks(user_id):
    profile = XMProfile.add(user_id)
    bankcards = profile.bankcards.get_all(Partner.xm)

    def never_used(bank):
        return all(
            not XMOrder.is_bankcard_swiped(bankcard)
            for bankcard in bankcards if bankcard.bank is bank)

    for bank in bank_collection.banks:
        if Partner.xm not in bank.available_in:
            continue
        if never_used(bank):
            amount_limit = bank.xmlib_amount_limit[0]
        else:
            amount_limit = bank.xmlib_amount_limit[1]
...
def get_expect_payback_date(due_date):
    """从到期日时间计算出赎回日，包含了法定节假日等非到期日顺延规则."""
